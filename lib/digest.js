...
function respondChallenge(req, res, status, realm, qop, nonceValue, srf) {
  const headers = {} ;
...
function digest(opts) {
  ...
  return function(req, res, next) {
    ...
    if (!req.authorization || Object.keys(req.authorization).length === 0) {
      ...
      return respondChallenge(req, res, opts.proxy ? 407 : 401, realmSelected, qop, nonceValue, srf) ;
    }
    ...
    passwordLookup(auth.username, auth.realm, (err, password) => {
      if (err) {
        const nonceValue = nonce() ;
        return respondChallenge(req, res, opts.proxy ? 407 : 401, realm, qop, nonceValue, srf) ;
      }
      else if (!password) {
